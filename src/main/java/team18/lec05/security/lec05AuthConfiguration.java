...
@Configuration
@EnableWebSecurity
public class lec05AuthConfiguration extends WebSecurityConfigurerAdapter {
  ...
  protected void configure(AuthenticationManagerBuilder auth) throws Exception {
    ...
    auth.inMemoryAuthentication().withUser("cus1").password(passwordEncoder().encode("p@ss1")).roles("CUSTOMER");
    auth.inMemoryAuthentication().withUser("cus2")
        .password("$2y$10$8IbzoKwqlCJf.z8/7YThKuSB1nGAQSr8rtHN7pQzm4mx9nrOhsN1C").roles("CUSTOMER");
    auth.inMemoryAuthentication().withUser("seller").password(passwordEncoder().encode("p@ss")).roles("SELLER");
  }
